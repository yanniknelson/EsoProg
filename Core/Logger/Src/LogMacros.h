...
#include <stdlib.h>                           // for std::abort
...
#define LOG(logger, log_level, ...)                                                                                 \
    ...
    {                                                                                                               \
        if (logger)                                                                                                 \
        {                                                                                                           \
            if (TSPDLoggerPtr pSPDLogger = logger->get(); pSPDLogger && pSPDLogger->should_log(log_level))          \
            {                                                                                                       \
                pSPDLogger->log(spdlog::source_loc{ __FILE__, __LINE__, SPDLOG_FUNCTION }, log_level, __VA_ARGS__); \
            }                                                                                                       \
        }                                                                                                           \
    } while (0)
...
#define LOG_FATAL(logger, ...)                                                                                                    \
    ...
    {                                                                                                                             \
        if (logger)                                                                                                               \
        {                                                                                                                         \
            if (TSPDLoggerPtr pSPDLogger = logger->get(); pSPDLogger)                                                             \
            {                                                                                                                     \
                pSPDLogger->log(spdlog::source_loc{ __FILE__, __LINE__, SPDLOG_FUNCTION }, spdlog::level::critical, __VA_ARGS__); \
            }                                                                                                                     \
        }                                                                                                                         \
        spdlog::shutdown();                                                                                                       \
        std::abort();                                                                                                             \
