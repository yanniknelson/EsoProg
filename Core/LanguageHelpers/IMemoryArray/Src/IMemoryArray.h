...
class IMemoryArray
{
  public:
    ...
    //////////////////////////////////////////////////////////////
    friend std::ostream& operator<<(std::ostream& rOS, const IMemoryArray<memType>& arr)
    {
        for (typename std::map<int, memType>::const_iterator it : arr.m_array)
        {
            rOS << it->first << "," << it->second << std::endl;
        }
        return rOS;
    }
        ...
        if (ImGui::Begin("Memory Array"))
        {
            ...
            for (size_t cellNum = 0; cellNum < numCells; cellNum++)
            {
                ...
                if (cellIndex == index)
                {
                    ...
                    pDrawList->AddTriangleFilled(traingleTopLeft, trainglePoint, traingleTopRight, ImColor(rStyle.Colors[ImGuiCol_Text]));
                ...
                pDrawList->AddText(cellIndexTextPos, ImColor(rStyle.Colors[ImGuiCol_Text]), std::to_string(cellIndex).c_str());
                pDrawList->AddRectFilled(rectMin, rectMax, ImColor(rStyle.Colors[ImGuiCol_FrameBg]), 4.0f);
                pDrawList->AddRect(rectMin, rectMax, ImColor(rStyle.Colors[ImGuiCol_Border]), 4.0f);
                ...
                pDrawList->AddText(valTextPos, ImColor(rStyle.Colors[ImGuiCol_Text]), valStr.c_str());
            }
        }
        ImGui::End();
    }
